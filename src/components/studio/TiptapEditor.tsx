...
function Btn({ active, onClick, title, children, disabled }: { active?: boolean; onClick: () => void; title: string; children: React.ReactNode; disabled?: boolean }) {
  return (
    <button
      ...
      title={title}
      disabled={disabled}
      className={`p-2 rounded-lg transition-all duration-300 disabled:opacity-10 ${active ? 'bg-[#C56A4A] text-black' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
    >
      {children}
    </button>
  );
