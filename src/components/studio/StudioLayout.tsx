...
export default function StudioLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#FFFFFF' }}>
      ...
      {/* Mobile top bar */}
      <header className="lg:hidden fixed top-0 inset-x-0 z-40 flex items-center justify-between px-4 h-14 border-b" style={{ background: '#000000', borderColor: '#222' }}>
        <Link to="/studio" className="font-outfit font-semibold" style={{ color: '#FFFFFF' }}>Studio</Link>
        <button onClick={() => setOpen(true)} aria-label="Menu"><Menu size={22} color="#FFFFFF" /></button>
      </header>

      {/* Sidebar */}
      <aside className={`${open ? 'fixed inset-0 z-50' : 'hidden'} lg:relative lg:block lg:w-64 lg:shrink-0`}>
        <div className={`${open ? 'flex' : 'hidden lg:flex'} flex-col h-screen w-64 sticky top-0`} style={{ background: '#000000', color: '#FFFFFF' }}>
          <div className="flex items-center justify-between px-5 py-5 border-b" style={{ borderColor: '#1a1a1a' }}>
            <Link to="/studio" className="font-outfit text-lg font-semibold" onClick={() => setOpen(false)}>
              Retention<span style={{ color: '#00D4FF' }}>.</span>Studio
            </Link>
          ...
          <nav className="flex-1 px-3 py-4 space-y-1">
            {items.map((i) => (
              <NavLink
                ...
                style={({ isActive }: any) => isActive ? { background: '#00D4FF', color: '#000000', fontWeight: 600 } : { color: 'rgba(255,255,255,0.75)' }}
              >
                <i.icon size={16} />
                {i.label}
              </NavLink>
          ...
          <div className="px-3 py-4 border-t" style={{ borderColor: '#1a1a1a' }}>
            <div className="px-3 mb-3">
              <p className="text-xs font-inter truncate" style={{ color: 'rgba(255,255,255,0.5)' }}>{user?.email}</p>
              <p className="text-[10px] uppercase tracking-wider mt-0.5" style={{ color: isAdmin ? '#00D4FF' : '#10B981' }}>{isAdmin ? 'Admin' : 'Editor'}</p>
            </div>
            <button
