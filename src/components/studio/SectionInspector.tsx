...
function FieldEditor({ field, value, onChange }: { field: Field; value: unknown; onChange: (v: unknown) => void }) {
  ...
  const inputClasses = "w-full px-5 py-4 rounded-2xl bg-white/[0.02] border border-white/10 text-white text-sm font-medium focus:outline-none focus:border-[#C56A4A]/50 transition-all placeholder:text-white/5";
      ...
      <div className="mb-8 last:mb-0">
        <label className="flex items-center gap-3 cursor-pointer group">
          <div className="relative">
            ...
            <div className={`w-10 h-6 rounded-full transition-colors duration-300 ${value ? 'bg-[#C56A4A]' : 'bg-white/10'}`} />
      ...
      <div className="mb-8 last:mb-0">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col gap-2">
            <button type="button" onClick={() => setPickerOpen(true)} className="px-6 py-3 rounded-xl bg-[#C56A4A] text-black text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all duration-500">Inject</button>
