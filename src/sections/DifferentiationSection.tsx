    ...
    <section ref={sectionRef} className="relative overflow-hidden" style={{ background: '#0A0A0A', padding: '12vh clamp(20px, 5vw, 80px) 14vh' }}>
      ...
      <div className="relative max-w-[1000px] mx-auto">
        <div className="diff-content mb-10 sm:mb-14" style={{ opacity: 0 }}>
          ...
          <h2 className="font-outfit font-medium mb-5 sm:mb-6" style={{ fontSize: 'clamp(26px, 5vw, 56px)', lineHeight: 1, color: '#EBE8E0', letterSpacing: '-0.02em' }}>
            We're Not Another "Email Marketing Agency"
          </h2>
          <p className="font-inter" style={{ fontSize: 'clamp(15px, 2.5vw, 18px)', lineHeight: 1.7, color: 'rgba(235,232,224,0.7)', maxWidth: '550px' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          <div className="diff-card p-6 sm:p-8 rounded-xl sm:rounded-2xl" style={{ opacity: 0, background: 'rgba(239,68,68,0.04)', border: '1px solid rgba(239,68,68,0.1)' }}>
            <h4 className="font-outfit font-medium mb-4 sm:mb-5" style={{ fontSize: '14px', color: 'rgba(235,232,224,0.5)', letterSpacing: '0.02em', textTransform: 'uppercase' }}>
              We Don't Focus On
            </h4>
            <div className="flex flex-col gap-3">
              {['Open rates', 'Click rates'].map((item) => (
                <div key={item} className="flex items-center gap-3">
                  ...
                  <span className="font-inter" style={{ fontSize: 'clamp(14px, 2vw, 16px)', color: 'rgba(235,232,224,0.5)', textDecoration: 'line-through' }}>{item}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-col gap-3">
              {['Revenue per customer', 'Lifetime value', 'Retention-driven growth'].map((item) => (
                <div key={item} className="flex items-center gap-3">
                  ...
                  <span className="font-inter font-medium" style={{ fontSize: 'clamp(14px, 2vw, 16px)', color: '#EBE8E0' }}>{item}</span>
                </div>
              ))}
            </div>
          </div>
