...
export default function SolutionSection() {
  ...
  return (
    <section ref={sectionRef} className="relative" style={{ background: '#0a0f1a', padding: '12vh clamp(20px, 5vw, 80px) 10vh' }}>
      <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr,1.1fr] gap-10 lg:gap-14 items-center">
        ...
        <div className="order-1 lg:order-2">
          <div className="solution-head mb-8 sm:mb-10" style={{ opacity: 0 }}>
            ...
            <h2 className="font-outfit font-medium mb-4 sm:mb-5" style={{ fontSize: 'clamp(28px, 5vw, 56px)', lineHeight: 1, color: '#F8FAFC', letterSpacing: '-0.02em' }}>
              {c.headline}
            </h2>
            <p className="font-inter" style={{ fontSize: 'clamp(15px, 2.5vw, 18px)', lineHeight: 1.7, color: '#94A3B8', maxWidth: '540px' }}>{c.body}</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {(c.benefits || []).map((b, i) => (
              <div key={i} className="solution-card p-4 sm:p-5 rounded-xl" style={{ opacity: 0, background: '#111827', border: '1px solid rgba(255,255,255,0.08)' }}>
                <div className="flex items-start gap-3">
                  ...
                  <span className="font-inter" style={{ fontSize: 'clamp(13px, 1.9vw, 15px)', color: '#F8FAFC', lineHeight: 1.5 }}>{b.text}</span>
                </div>
              </div>
            ))}
          </div>
