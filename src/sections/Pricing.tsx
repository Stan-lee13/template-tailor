...
function PricingCard({ plan, large = false, onSelect }: { plan: typeof plans[0]; large?: boolean; onSelect: () => void }) {
  ...
  return (
    <ThreeDCard className="h-full">
      <ThreeDCardItem translateZ={30} className={`relative rounded-[2.5rem] overflow-hidden h-full flex flex-col p-8 lg:p-12 ${isFeatured ? 'bg-gradient-to-br from-[#C9A227]/20 to-black border-2 border-[#C9A227]/30 shadow-[0_0_50px_rgba(201, 162, 39,0.1)]' : 'bg-white/5 border border-white/10 hover:border-white/20'} transition-all duration-500`}>
        {isFeatured && (
          <div className="absolute top-0 right-0 px-6 py-2 bg-[#C9A227] text-black text-xs font-black uppercase tracking-widest rounded-bl-2xl">
            Most Popular
          </div>
        )}
        ...
        <div className="space-y-4 mb-12 flex-1">
          {plan.features.map((feature) => (
            <div key={feature} className="flex items-start gap-4">
              <div className="flex-shrink-0 w-5 h-5 rounded-full bg-[#C9A227]/10 flex items-center justify-center text-[#C9A227]">
        ...
        <button
          onClick={onSelect}
          className={`w-full py-5 rounded-2xl font-bold text-sm transition-all duration-500 ${isFeatured ? 'bg-[#C9A227] text-black hover:bg-white hover:shadow-[0_0_30px_rgba(255,255,255,0.3)]' : 'bg-white/10 text-white hover:bg-white hover:text-black'} transform hover:-translate-y-1`}
        >
          Get Started Now
        </button>
      </ThreeDCardItem>
...
export default function Pricing() {
  ...
  return (
    <section ref={sectionRef} id="pricing" className="relative" style={{ background: '#0B1A2A', padding: '14vh clamp(20px, 5vw, 80px) 12vh' }}>
      {/* Subtle separator */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[400px] h-[1px]" style={{ background: 'linear-gradient(90deg, transparent, rgba(201, 162, 39,0.2), transparent)' }} />

      <div className="relative max-w-[1100px] mx-auto">
        <div className="pricing-head mb-10 sm:mb-14 text-center" style={{ opacity: 0 }}>
          <span className="block font-inter font-medium uppercase mb-5 sm:mb-6" style={{ fontSize: '13px', color: '#C9A227', letterSpacing: '0.15em' }}>
            Pricing
          </span>
        ...
        <p className="font-inter text-center mt-8 sm:mt-10" style={{ fontSize: 'clamp(14px, 2vw, 15px)', color: 'rgba(255,255,255,0.4)' }}>
          Not sure which plan?{' '}
          <button onClick={() => select('not_sure')} className="font-medium underline underline-offset-2 transition-colors duration-300" style={{ color: '#C9A227' }}
