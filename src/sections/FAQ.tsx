      ...
      <button onClick={onClick} className="w-full flex items-center justify-between py-5 sm:py-6 text-left transition-colors duration-200">
        ...
        <span className="flex-shrink-0 transition-transform duration-300" style={{ transform: isOpen ? 'rotate(45deg)' : 'rotate(0deg)' }}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            ...
            <line x1="0" y1="8" x2="16" y2="8" stroke={isOpen ? '#F97316' : '#F8FAFC'} strokeWidth="2" />
          </svg>
        </span>
      </button>
      <div className="overflow-hidden transition-all duration-400" style={{ maxHeight: isOpen ? '400px' : '0', opacity: isOpen ? 1 : 0 }}>
        <p className="font-inter pb-5 sm:pb-6" style={{ fontSize: 'clamp(14px, 2vw, 16px)', lineHeight: 1.6, color: '#94A3B8' }}>{answer}</p>
      </div>
    </div>
  );
}
...
export default function FAQ() {
  ...
  return (
    <section ref={sectionRef} id="faq" style={{ background: '#0a0f1a', padding: '10vh clamp(20px, 5vw, 80px) 12vh' }}>
      <div className="max-w-[720px] mx-auto">
        ...
        <h2 className="text-center font-outfit font-medium mb-10 sm:mb-16" style={{ fontSize: 'clamp(28px, 5vw, 60px)', lineHeight: 0.95, color: '#F8FAFC', letterSpacing: '-0.02em' }}>{c.headline}</h2>
        <div>
          {(c.faqs || []).map((faq, i) => (
