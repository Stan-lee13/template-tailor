...
function AccordionItem({ question, answer, isOpen, onClick }: { question: string; answer: string; isOpen: boolean; onClick: () => void }) {
  return (
    <div style={{ borderBottom: '1px solid #D6D3CC' }}>
      <button onClick={onClick} className="w-full flex items-center justify-between py-6 text-left transition-colors duration-200">
        <span className="font-inter pr-4" style={{ fontSize: '17px', lineHeight: 1.4, color: '#0A0A0A' }}>{question}</span>
      ...
      <div className="overflow-hidden transition-all duration-400" style={{ maxHeight: isOpen ? '300px' : '0', opacity: isOpen ? 1 : 0 }}>
        <p className="font-inter pb-6" style={{ fontSize: '16px', lineHeight: 1.6, color: '#2D2D2D' }}>{answer}</p>
      </div>
    </div>
  );
}
...
export default function FAQ() {
  ...
  return (
    <section ref={sectionRef} id="faq" style={{ background: '#EBE8E0', padding: '12vh clamp(24px, 5vw, 80px)' }}>
      <div className="max-w-[720px] mx-auto">
        ...
        <h2 className="text-center font-outfit font-medium mb-16" style={{ fontSize: 'clamp(30px, 5vw, 60px)', lineHeight: 0.95, color: '#0A0A0A', letterSpacing: '-0.02em' }}>
          Common Questions
        </h2>
        <div>
          {faqs.map((faq, i) => (
