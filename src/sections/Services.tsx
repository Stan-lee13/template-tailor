            ...
            <div className="absolute inset-0 flex items-center justify-center">
              {services.map((s, i) => (
                <article key={i} className="svc-card absolute" style={{ width: 'min(440px, 32vw)', willChange: 'transform, opacity' }} aria-current={i === activeIdx}>
                  <div className="p-8 rounded-3xl" style={{ background: '#111827', border: `1px solid ${s.accent}30`, boxShadow: `0 30px 80px ${s.accent}20, 0 0 0 1px rgba(0,0,0,0.02)` }}>
                    <span className="font-outfit font-medium" style={{ fontSize: '13px', color: s.accent }}>{s.number}</span>
                    <h3 className="font-outfit font-medium mt-3 mb-5" style={{ fontSize: '26px', color: '#F8FAFC', letterSpacing: '-0.01em', lineHeight: 1.15 }}>{s.title}</h3>
                    <div className="flex flex-wrap gap-2">
                      {(s.items || '').split(',').map((item) => item.trim()).filter(Boolean).map((item) => (
                        <span key={item} className="font-inter px-3 py-1.5 rounded-full" style={{ fontSize: '13px', background: s.accent + '10', color: '#94A3B8', border: `1px solid ${s.accent}20` }}>{item}</span>
                      ))}
                    </div>
                  </div>
                </article>
      ...
      <div className="lg:hidden relative" style={{ padding: '10vh 20px 12vh' }}>
        <div className="max-w-[720px] mx-auto">
          ...
          <div className="flex flex-col gap-3">
            {services.map((s, i) => (
              <div key={i} className="svc-card-mobile p-5 rounded-2xl" style={{ opacity: 0, background: '#111827', border: '1px solid rgba(255,255,255,0.08)' }}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-outfit font-medium mb-3" style={{ fontSize: '18px', color: '#F8FAFC', letterSpacing: '-0.01em' }}>{s.title}</h3>
                    <div className="flex flex-wrap gap-1.5">
                      {(s.items || '').split(',').map((item) => item.trim()).filter(Boolean).map((item) => (
                        <span key={item} className="font-inter px-2.5 py-1 rounded-full" style={{ fontSize: '12px', background: s.accent + '10', color: '#94A3B8', border: `1px solid ${s.accent}20` }}>{item}</span>
                      ))}
                    </div>
                  </div>
                </div>
