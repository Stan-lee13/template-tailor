...
export default function VisualEditor() {
  ...
  return (
    <div className="h-screen w-screen flex flex-col overflow-hidden bg-black selection:bg-[#C9A227] selection:text-black">
      <Helmet>
      ...
      <header className="flex items-center justify-between px-6 h-16 border-b border-white/5 shrink-0 bg-black">
        <div className="flex items-center gap-6 min-w-0">
          ...
          <div className="text-sm font-black tracking-tighter text-white">
            VISUAL<span className="text-[#C9A227]">.</span>ENGINE
          </div>
          <div className="h-4 w-px bg-white/10 hidden md:block" />
          <select
            value={pageId || ''}
            onChange={(e) => setPageId(e.target.value)}
            className="hidden md:block px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest bg-white/5 text-white border-none focus:ring-1 focus:ring-[#C9A227]/50 transition-all cursor-pointer"
          >
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex p-1 rounded-2xl bg-white/5 border border-white/5">
            {(['desktop', 'tablet', 'mobile'] as const).map((d) => (
              <button
                ...
                style={{ 
                  background: device === d ? '#C9A227' : 'transparent', 
          ...
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/5 text-[10px] font-black uppercase tracking-widest text-white/40">
            {saveState === 'saving' && (<><Loader2 size={12} className="animate-spin text-[#C9A227]" /> Synchronizing</>)}
          ...
          {canEdit && (
            <button
              onClick={publishNow}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest bg-[#C9A227] text-black hover:bg-white transition-all duration-500 shadow-[0_0_20px_rgba(201, 162, 39,0.2)]"
            >
