...
export default function ActivityLog() {
  ...
  return (
    <StudioLayout>
      <div className="mb-12">
        <h1 className="text-4xl lg:text-6xl font-black text-white tracking-tighter mb-4">System <span className="text-gradient-warm">Logs</span></h1>
          ...
          <ul className="divide-y divide-white/5">
            {logs.map((l) => (
              <li key={l.id} className="px-8 py-6 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8 hover:bg-white/[0.02] transition-colors group">
                <span className="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-lg bg-white/5 text-[#C56A4A] border border-white/5 group-hover:border-[#C56A4A]/30 transition-all">{l.action}</span>
