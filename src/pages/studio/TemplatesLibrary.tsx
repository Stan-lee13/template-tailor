...
export default function TemplatesLibrary() {
  ...
  return (
    <StudioLayout>
      <div className="mb-12">
        <h1 className="text-4xl lg:text-6xl font-black text-white tracking-tighter mb-4">Preset <span className="text-gradient-warm">Blueprints</span></h1>
      ...
      {loading ? (
        <div className="flex items-center gap-3 text-white/20 font-black text-xs uppercase tracking-widest">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((t) => (
              <li key={t.id} className="group p-8 rounded-[2.5rem] bg-white/[0.03] border border-white/10 hover:border-[#C56A4A]/30 transition-all duration-500 flex flex-col justify-between">
                <div className="min-w-0 mb-8">
                  ...
                  <h3 className="text-xl font-black text-white tracking-tight truncate group-hover:text-[#C56A4A] transition-colors">{t.name}</h3>
