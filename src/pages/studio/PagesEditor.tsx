  ...
  return (
    <StudioLayout>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-12">
        <div>
          <h1 className="text-4xl lg:text-6xl font-black text-white tracking-tighter mb-4">Route <span className="text-gradient-warm">Intelligence</span></h1>
          <p className="text-white/40 font-medium">SEO and metadata parameters for every system route.</p>
        </div>
        <button
          onClick={createPage}
          className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest bg-[#C56A4A] text-black hover:bg-white transition-all duration-500 shadow-[0_0_20px_rgba(197,106,74,0.2)]"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px,1fr] gap-8">
            ...
            <div className="divide-y divide-white/5">
              {pages.map((p) => (
                <button
                  ...
                  className={`w-full text-left px-8 py-6 transition-all duration-500 ${selected?.id === p.id ? 'bg-white/5 border-l-4 border-l-[#C56A4A]' : 'hover:bg-white/[0.02]'}`}
                >
        ...
        {selected ? (
          <div className="rounded-[2.5rem] p-10 lg:p-12 bg-black border border-white/10 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-64 h-64 bg-[#C56A4A]/5 blur-[80px] rounded-full -translate-y-1/2 translate-x-1/2" />

            <div className="flex items-center justify-between gap-6 mb-12 relative z-10">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#C56A4A] mb-2">{selected.path}</p>
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={save}
                  disabled={saving}
                  className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest bg-white text-black hover:bg-[#C56A4A] transition-all duration-500 shadow-[0_0_20px_rgba(255,255,255,0.1)] disabled:opacity-50"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
              <div className="space-y-3">
                ...
                <input
                  className="w-full px-6 py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-black text-sm focus:outline-none focus:border-[#C56A4A]/50 transition-all duration-300"
              ...
              <div className="space-y-3">
                ...
                <select
                  className="w-full px-6 py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-black text-xs uppercase tracking-widest focus:outline-none focus:border-[#C56A4A]/50 transition-all cursor-pointer"
              ...
              <div className="md:col-span-2 space-y-3">
                ...
                <div className="relative">
                  <textarea
                    rows={3}
                    className="w-full px-6 py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-medium text-sm focus:outline-none focus:border-[#C56A4A]/50 transition-all duration-300 resize-none"
          ...
          <div className="rounded-[2.5rem] p-20 text-center bg-black border border-white/10">
            <div className="w-16 h-16 rounded-[2rem] bg-white/5 border border-white/10 flex items-center justify-center mx-auto mb-8 animate-pulse">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-[#C56A4A]"><path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"/><polyline points="13 2 13 9 20 9"/></svg>
            </div>
