    ...
    <StudioLayout>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-12">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-black text-white tracking-tight">Content <span className="text-[#C9A227]">Architect</span></h1>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <Sparkles size={12} className="text-[#C9A227]" /> AI Assistant
          </button>
          ...
          {p.status === 'published' ? (
            ...
            <button 
              ...
              className="px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest bg-[#C9A227] text-black hover:bg-white transition-all duration-500 shadow-[0_0_20px_rgba(201, 162, 39,0.2)]"
            >
              Deploy Live
            </button>
          )}
      ...
      <div className="flex gap-4 mb-10 p-1.5 rounded-2xl bg-white/5 border border-white/5 w-fit">
        {(['content', 'seo', 'settings'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-6 py-2 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] transition-all duration-300 ${tab === t ? 'bg-[#C9A227] text-black' : 'text-white/30 hover:text-white'}`}
          >
            {t}
          </button>
        ))}
            ...
            <ul className="space-y-4">
              {checklist.map((c) => (
                <li key={c.id} className="flex items-start gap-3">
                  <span className="mt-0.5">{c.pass ? <Check size={16} className="text-[#C9A227]" strokeWidth={3} /> : <X size={16} className="text-rose-500" strokeWidth={3} />}</span>
          ...
          <Field label="Open Graph image">
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-4">
                <button onClick={() => ogRef.current?.click()} className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl bg-[#C9A227] text-black text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all duration-500 shadow-[0_0_20px_rgba(201, 162, 39,0.2)]">
        ...
        <div className="space-y-8 max-w-2xl">
          ...
          <Field label="Featured image">
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-4">
                <button onClick={() => fileRef.current?.click()} className="inline-flex items-center gap-3 px-8 py-4 rounded-2xl bg-[#C9A227] text-black text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all duration-500 shadow-[0_0_20px_rgba(201, 162, 39,0.2)]">
          ...
          <Field label="Schedule publication">
            ...
            {p.scheduled_for && p.status !== 'scheduled' && (
              <button onClick={handleSchedule} className="mt-4 px-8 py-4 rounded-2xl bg-[#C9A227] text-black text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all duration-500 shadow-[0_0_20px_rgba(201, 162, 39,0.2)]">Schedule Pipeline</button>
            )}
          </Field>
        </div>
      )}
          ...
          <div className="w-full max-w-md bg-black border border-white/10 rounded-[3rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,1)]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-10 space-y-6">
              <ul className="space-y-4">
                {checklist.map((c) => (
                  <li key={c.id} className="flex items-start gap-3">
                    {c.pass ? <Check size={16} className="text-[#C9A227] mt-0.5" strokeWidth={3} /> : <X size={16} className="text-rose-500 mt-0.5" strokeWidth={3} />}
                    <span className="text-xs font-black uppercase tracking-widest text-white/40">{c.label}{c.hint && <span className="text-white/10 lowercase tracking-normal font-medium"> · {c.hint}</span>}</span>
                  </li>
                ))}
              </ul>
              <div className="flex gap-4 pt-4">
                ...
                <button onClick={() => { setShowChecklist(false); setTab('seo'); }} className="flex-1 px-6 py-4 rounded-2xl bg-[#C9A227] text-black text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all duration-500">Fix Protocol</button>
              </div>
            </div>
          </div>
        </div>
...
const inputCls = 'w-full px-6 py-4 rounded-2xl bg-white/[0.02] border border-white/10 text-white font-black text-sm focus:outline-none focus:border-[#C9A227]/50 transition-all duration-300 placeholder:text-white/10';
