    ...
    >
      <div className="mt-20 space-y-24">
        <section className="space-y-12">
          <div className="inline-block px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-[0.3em] text-[#00D4FF]">Ideal Entity Profile</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            {[
              ...
              <div key={i} className="p-10 rounded-[2.5rem] bg-white/[0.03] border border-white/10 hover:border-[#00D4FF]/30 transition-all duration-500 group">
                <h3 className="text-xl font-black text-white uppercase tracking-tight mb-4 group-hover:text-[#00D4FF] transition-colors">{role.title}</h3>
        ...
        <section className="p-10 lg:p-20 rounded-[3rem] bg-white/[0.02] border border-white/10 relative overflow-hidden text-center">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-96 h-96 bg-[#00D4FF]/5 blur-[100px] rounded-full -translate-y-1/2" />
          <div className="relative z-10">
            ...
            <p className="text-lg lg:text-xl font-medium text-white/40 max-w-2xl mx-auto leading-relaxed mb-12">
              Send a concise transmission regarding your past deployments and future trajectory to <a href={`mailto:${SITE.email}`} className="text-white hover:text-[#00D4FF] transition-colors">{SITE.email}</a>. 
